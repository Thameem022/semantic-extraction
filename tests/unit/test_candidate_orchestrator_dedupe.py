...
def test_deduplicate_prefers_kv_for_protected_fields_when_sources_conflict():
    """Core applicant fields must keep KV when overlapping with Table/LLM-TABLE."""
    kv_website = _candidate(
        field_id="website",
        source="KV",
        raw_value="https://kv.example.com",
        bounding_box=[0.0, 0.0, 10.0, 0.0, 10.0, 10.0, 0.0, 10.0],
    )
    llm_website = _candidate(
        field_id="website",
        source="LLM-TABLE",
        raw_value="https://wrong.example.com",
        confidence=0.85,
        bounding_box=[0.5, 0.5, 9.5, 0.5, 9.5, 9.5, 0.5, 9.5],
    )

    deduped = _deduplicate_candidates([llm_website, kv_website])

    assert len(deduped) == 1
    assert deduped[0].rawValue == "https://kv.example.com"
    assert "KV" in deduped[0].source
